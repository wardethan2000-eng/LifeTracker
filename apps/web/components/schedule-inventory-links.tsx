...
import type { InventoryItemSummary, ScheduleInventoryLinkDetail } from "@aegis/types";
...
export function ScheduleInventoryLinks({
  ...
  const inventoryCategoryLookup = useMemo(
    () => new Map(inventoryItems.map((item) => [item.id, item.category ?? null])),
    [inventoryItems],
  );
      ...
      {isLoading ? (
        ...
        <>
          <CategoryAccordionList
            items={links}
            getSearchText={(link) =>
              [link.inventoryItem.name, link.inventoryItem.partNumber ?? ""].join(" ")
            }
            getCategory={(link) => inventoryCategoryLookup.get(link.inventoryItemId)?.trim() || null}
            searchPlaceholder="Filter required parts by item or part number"
            emptyMessage="No required parts linked yet."
            noMatchMessage="No required parts match that search."
