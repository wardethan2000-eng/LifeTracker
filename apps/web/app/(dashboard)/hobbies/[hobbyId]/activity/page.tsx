...
type HobbyActivityPageProps = {
  params: Promise<{ hobbyId: string }>;
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

const formatActionLabel = (value: string): string =>
  value.split(/[._]/g).filter(Boolean).map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(" ");
...
const renderEntityRef = (entry: ActivityLog): JSX.Element => {
  ...
  return (
    <p style={{ color: "var(--ink-muted)", fontSize: "0.88rem", display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
      ...
      {url ? (
        <Link href={url} className={`activity-entity-link${deleted ? " activity-entity-link--deleted" : ""}`}>{name}</Link>
      ) : (
        <span title={entry.entityId} style={deleted ? { textDecoration: "line-through" } : undefined}>{name}</span>
      )}
...
export default async function HobbyActivityPage({ params, searchParams }: HobbyActivityPageProps): Promise<JSX.Element> {
  const [{ hobbyId }, sp] = await Promise.all([params, searchParams]);
...
async function ActivityContent({ householdId, hobbyId, cursor, history }: { householdId: string; hobbyId: string; cursor?: string; history: string[] }): Promise<JSX.Element> {
  ...
  const buildHref = (p: { cursor?: string; history?: string[]; limit: number }): string => {
    ...
    return `/hobbies/${hobbyId}/activity?${q.toString()}`;
  };
      ...
      <div className="panel__body">
        {activity.entries.length === 0 ? (
          <div className="empty-state">
            ...
            <p className="empty-state__body">Changes and events for this hobby will appear here over time.</p>
          </div>
        ) : (
                ...
                {entry.metadata ? (
                  <div style={{ display: "grid", gap: "8px" }}>
                    {Object.entries(entry.metadata).map(([key, value]) => (
                      <div key={key}>
                        <div style={{ fontSize: "0.76rem", color: "var(--ink-muted)", textTransform: "uppercase", letterSpacing: "0.04em" }}>{formatMetadataLabel(key)}</div>
