...
export default async function AssetDetailPage({ params, searchParams }: AssetDetailPageProps): Promise<JSX.Element> {
  ...
  try {
    ...
    const [libraryPresets, customPresets, householdAssets, householdMembers, metricInsights, metricCorrelations, comments, transferHistory, historyTimeline, overviewTimeline, costSummary, costPerUnit, costForecast] = await Promise.all([
      getLibraryPresets(),
      ...
      tab === "costs" ? getAssetCostSummary(assetId).catch(() => null) : Promise.resolve(null),
      tab === "costs" ? getAssetCostPerUnit(assetId).catch(() => null) : Promise.resolve(null),
      tab === "costs" ? getAssetCostForecast(assetId).catch(() => null) : Promise.resolve(null)
    ]);
